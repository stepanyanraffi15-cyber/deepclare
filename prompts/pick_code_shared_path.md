...
Each is a real entry of the current nomenclature, identified by its full path from chapter
down to the entry itself, because leaf names are elliptical — "other", "not more than
100 V" — and mean nothing without the branch above them.

The search was scoped, so the candidates share the opening of that path. It is stated once
here rather than repeated on every row:

{{shared_path}}

Every row below continues from that shared opening. A row's full path is the shared opening
above, then `›`, then the row's own text — which is the part that tells the candidates
apart. When the shared opening reads `(none)` the candidates have none in common and each
row already carries its whole path.
